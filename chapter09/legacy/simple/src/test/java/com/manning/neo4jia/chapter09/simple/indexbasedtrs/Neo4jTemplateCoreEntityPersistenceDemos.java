...
import com.manning.neo4jia.chapter09.simple.indexbasedtrs.domain.Viewing;
import com.manning.neo4jia.chapter09.simple.indexbasedtrs.domain.dodgy.UnannotatedUser;
import com.manning.neo4jia.chapter09.simple.indexbasedtrs.util.GraphUtil;
...
public class Neo4jTemplateCoreEntityPersistenceDemos {
    ...
    public void demoSaveUserAndThenFindByNodeId() {
        ...
        // Listing 9.7 Neo4jTemplate (shows the case where the lookup
    ...
    public void demoSaveUserAndThenLookupByIndexedUserId() {
        ...
        User loadedUserViaIndex = template
                .lookup("User","userId","john001")
                .to(User.class).singleOrNull();
        assertEquals("User retrieved via index lookup does not match saved user",user, loadedUserViaIndex);
    }

    @Test
    @Transactional
    public void demoSaveUserAndThenFindUsingNativeNeo4JIndexAPI() {

        // This corresponds closely to the code detailed in the sidebar box
        // titled - Under what index names are these annotated properties
        // stored?
        User user      = new User("john001","John");
        template.save(user);
    ...
    public void demoSavingOneSDNEntityCanTransitivelyResultInOtherAnotherEntityBeingSaved() {
        ...
        Index<Node> userIdIndex = indexManager.forNodes("User");
        IndexHits<Node> indexHits = userIdIndex.get("userId","john001");
        Node loadedJohnNode = indexHits.getSingle();
        indexHits = userIdIndex.get("userId","susan001");
        Node loadedSusanNode = indexHits.getSingle();
        assertNotNull("Should have been able to lookup john after saving",loadedJohnNode);
        assertNotNull("Should have been able to lookup susan after saving john",loadedSusanNode);
